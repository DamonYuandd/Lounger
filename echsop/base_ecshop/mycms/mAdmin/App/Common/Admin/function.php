...
function isLang() {
	...
	$pid = 4;
	$langList = $categoryDao->where(array('pid'=>$pid,'is_publish'=>1,'alias'=>$_SESSION['lang']))->order('is_default desc,ordernum desc')->find();
	echo '当前语言：'.$langList['title'];
}
...
//返回分类标题
function getCategoryTitle($id,$bool = false) {
	$categoryDao = M('Category');
	if($bool == false){
		echo $categoryDao->getField('title', array('id'=>$id));}
	else{
		$obj = $categoryDao->where(array('id'=>$id))->find();
		return $obj;
	}
}
...
function getCurCategoryNav( $cid ) {
	if( !empty($cid) ) {
		...
		echo '<span>'.$nav_site.$category['title'].'</span>';
	}
}
	...
	if( count($langList)>1 ) {
		...
		foreach ($langList as $key => $value) {
			if($value['alias'] == $rmoveLang ) {
				...
				$input .= '<input type="checkbox" name="synch_lang[]" id="checkbox-'.$pnum.'" class="custom" value="'.$value['alias'].'" /><label for="checkbox-'.$pnum.'">'.$value['title'].'</label>';
				$pnum++;
			}
		}        
		echo '<label for="name">同步语言：</label> <fieldset data-role="controlgroup">'.$input.' </fieldset>';
	}
}
...
//默认语言导航标题
function getDefNavTitle($title_langs,$index=0,$bool=false) {
	if( is_array($title_langs[$index]) ) {
		if($bool == false){
			echo $title_langs[$index]['title'];
		}else{
			return $title_langs[$index]['title'];
		}
	}
}
