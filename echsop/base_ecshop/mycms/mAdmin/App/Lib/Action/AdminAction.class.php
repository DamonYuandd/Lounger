...
class AdminAction extends CommonAction {
	...
	function _initialize() {
		...
		$_SESSION ['hardware'] = 'mobile';
		//获取系统信息
		$this->systemInfo = systemInfo();
		$this->assign('system',$this->systemInfo);
		//获取所有语言
		$this->assign('user_langs',getDefultLang('mobile',true));
		//获取公共信息
		$this->commonInfo = commonInfo();
		$this->assign('common',$this->commonInfo);
		$this->assign('sysImage','http://img.huyionline.cn/default/default.jpg');
	}
	...
	protected function _img_uploads($folder='',$thumb=true,$width=300,$height=300) {
		...
		//	$upload->thumbMaxWidth = '1200,'.$width;
		//	$upload->thumbMaxHeight = '1200,'.$height;
	...
	protected function _dataPage($categoryDao, $cid, $where) {
		...
		$getTitle = getCategoryTitle($cid,true);
		$this->assign('headTitle',$getTitle['title']);
	...
	public function _oneContent($cid) {
		...
		$this->assign('headTitle',$news['title']);
	...
	//转到列表内容页面
	public function _listContent($categoryDao, $cid, $where) {
		$cid = $_REQUEST['cid'];
	...
	protected function _assignModuleList() {
		...
		$categoryDao = M('Category');
		$selectModute = array('About','News','Goods','Job','Guestbook');
		$notInalias = array('Contact','Weibo');
		$moduleList = $categoryDao->where(array('pid'=>12,'is_publish'=>1,'alias' => array('not in',$notInalias),'module'=>array('in',$selectModute)))->order('ordernum desc')->select();//选择模块
	...
	//设置操作提示后跳转的URL
	protected function setJumpUrl( $param ) {
		$this->assign('jumpUrl', __APP__.'/Admin/'.$param);
	}
