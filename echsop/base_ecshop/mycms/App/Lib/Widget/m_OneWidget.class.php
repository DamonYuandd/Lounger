<?php
//单页显示
class m_OneWidget extends Widget {
	...
	public function render( $data ){
		...
		if( !empty( $data['alias']) ) { //1.使用别名获取单页
			$category = $categoryDao->where( array('alias'=>$data['alias'], 'lang'=>$lang,'hardware' =>'mobile') )->find();
			if(!$category){
				$fid = $categoryDao->where( array('alias'=>$data['alias']) )->find();
				$category = $categoryDao->where(array('pid' => $fid['id'],'lang' => $lang, 'hardware' => 'mobile'))->order('ordernum desc')->find();
			}
			...
			if( ACTION_NAME == 'index' || ACTION_NAME == 'more' ) { //4-1.如果是主导航就获取主导航下第
				...
				$category = $categoryDao->where( array('pid'=>$category['id'], 'is_publish'=>1,'hardware' => 'mobile' , 'lang' => $lang) )->order('ordernum desc')->first();
			} else { //4-2.自动选择别名
