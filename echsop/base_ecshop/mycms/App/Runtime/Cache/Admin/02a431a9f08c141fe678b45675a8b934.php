	...
	<thead>
		<tr>
			<th width="600">
				...
				<?php if(($is_News)  !=  "1"): ?><input type="button" value="添加分类" onclick="goToUrl('__APP__/Admin/Category/editAdd/pid/<?php echo ($_SESSION['c_root']); ?>');" class="button-img-add" /><?php endif; ?>
				</div>
	...
	<tbody><?php $num = 0; ?>
		<?php if(is_array($categoryList)): $i = 0; $__LIST__ = $categoryList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><?php $num++;?>
			<tr>
		      ...
		      <td align="left">
		      <?php if(($is_News)  !=  "1"): ?><a href="__APP__/Admin/Category/editAdd/id/<?php echo ($vo["id"]); ?>">[添加子分类]</a><?php endif; ?>
		      &nbsp;&nbsp;[<a href="__APP__/Admin/Category/edit/pid/<?php echo ($vo["pid"]); ?>/c_root/<?php echo ($_SESSION['c_root']); ?>/id/<?php echo ($vo["id"]); ?>">编辑</a>]&nbsp;&nbsp;
		      <?php 
		          if($_SESSION['category_action']!='Market') {
		      ?>
		       <?php if(($is_News)  !=  "1"): ?>[<a href="#" onclick="deleteData('__APP__/Admin/Category/deleteCategory/id/<?php echo ($vo["id"]); ?>');">删除</a>]</td><?php endif; ?>
		      <?php 
		      }
		      ?>
		    </tr>
				...
				<tr>
			      ...
			      <td align="left">
			       <?php 
                      if($_SESSION['category_action']!='Market') {
                  ?>
			      <a href="__APP__/Admin/Category/editAdd/id/<?php echo ($child2["id"]); ?>" style="visibility:hidden;">[添加子分类]</a>
			      <?php 
                  }
                  ?>
