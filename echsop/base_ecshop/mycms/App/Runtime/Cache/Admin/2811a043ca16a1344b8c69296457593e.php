...
$(document).ready(function(){
		$("#thumbnails li input").live("blur",function(){
			var nowInput = $(this);
			var dataString = 't='+$(this).val()+'&img='+$(this).siblings('span').text();
			$.ajax({
				  url: "__APP__/Admin/Goods/ajaxUp", //登录处理页
                  dataType: "json",
                  data: dataString,
                  success:function(json){
						if(json == 1){
						//	alert();
							nowInput.siblings('p').html('已修改').show(500,function(){
								setTimeout(function(){
									nowInput.siblings('p').hide(500)
								}, 6000);
							});
							
						}else{
							nowInput.siblings('p').html('无修改').show(500,function(){
								setTimeout(function(){
									nowInput.siblings('p').hide(500)
								}, 6000);
							});
...
<script type="text/javascript">
		...
		window.onload = function() {
			var settings = {
				...
				upload_url: "__APP__/Admin/Goods/sufuLoad/cid/<?php echo $_GET['cid']; ?>/lang/<?php echo $_GET['lang']; ?>/hardware/<?php echo $_GET['hd']; ?>",
...
<body>
<div class="nav-site"><?php getNavSite($nav_site,$_GET['cid']);?> > 批量上传产品</div>
 <fieldset>
<div id="content">
			<strong style="color:orange;font-size:14px;">当前分类：<span style="color:red;"><?php getCategoryTitle($_GET['cid']);?></span></strong>
			<br/><br/>
			<strong style="color:orange;font-size:14px;">批量上传步骤：选择上传图片 -> 提交 -> 修改名称 ->返回产品列表</strong>
...
<div style="margin: 71px 90px;">
	  <input type="button" value="返回上一步" class="button button-green button-big"  onclick="javascript:history.go(-1);" />
</div>
</body>
</html>