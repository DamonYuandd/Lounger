...
class MBOpenTOAuth {
	public $host = 'http://opensns.qq.com/';
	public $timeout = 30; 
    ...
    function authenticateURL() { return 'http://openapi.qzone.qq.com/user/get_user_info'; } 
	/*function authenticateURL() { return 'http://openapi.qzone.qq.com/oauth/qzoneoauth_authenticate'; }*/ 
   ...
   */
     ...
     * @return string 
     */ 
    function getAuthorizeURL($key,$token, $signInWithWeibo = TRUE , $url='') { 
        if (is_array($token)) { 
            $token = $token['oauth_token']; 
        } 
        if (empty($signInWithWeibo)) { 
            return $this->authorizeURL() .  "?oauth_consumer_key={$key}&oauth_token={$token}&oauth_callback=" . urlencode($url);
        } else { 
            return $this->authenticateURL() . "?oauth_consumer_key={$key}&oauth_token={$token}&oauth_callback=" . urlencode($url);
        } 
	} 	
	...
	* Exchange the request token and secret for an access token and 
	...
	function getUserInfo($openid=FALSE, $oauth_token = false){
	  $parameters = array(); 
        if (!empty($openid)) { 
            $parameters['openid'] = $openid; 
        } 
		$request = $this->oAuthRequest($this->authenticateURL(), 'GET', $parameters);
        return $this->jsonDecode($request,true); 
	}
