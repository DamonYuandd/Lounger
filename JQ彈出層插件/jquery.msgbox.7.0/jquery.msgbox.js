...
(function($) {
	...
	$.msgbox = function(opts) {
		...
		this.mainWrap = $('<div />').css({
			...
			'top' : ($(window).height() - this.opts.height - 100) / 2 + 'px'
	...
	$.msgbox.resetposition = {
		...
		domready: function(){
			var _this = this;
			var offTop = parseInt(this.mainWrap.css('top'));
			var offLeft = parseInt(this.mainWrap.css('left'));
			...
			var resetPos = function(){
				...
				if( _this.relTop ){  // have been dragged
					_this.mainWrap.css({
						top: $(window).scrollTop() + _this.relTop + 'px',
						left: $(window).scrollLeft() + _this.relLeft + 'px' 
					});
				} else {
					_this.mainWrap.css({
						top: $(window).scrollTop() + offTop + 'px',
						left: $(window).scrollLeft() + offLeft + 'px' 
					});
				}
			}
	...
	$.msgbox.autoclose = {
		...
		domready: function(){
			...
			var _this = this;
			this.closeMsg = '&nbsp; &nbsp; Closing in <strong>{seconds}</strong>';
			var interval = null;
