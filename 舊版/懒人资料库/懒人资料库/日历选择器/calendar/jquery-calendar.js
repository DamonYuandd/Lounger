...
var popUpCal = {
	...
	formatDate : function(day, month, year, hour, minute) {
		...
		// append time info
		var seconds_s = new Date().getSeconds()
		if (this.timeSeparators && this.timeSeparators.length > 1
			...
			dateString += this.timeSeparators[0] + this.hourString(hour)
					...
					+ this.timeSeparators[1]
					+ ((seconds_s >= 10) ? seconds_s : ('0' + seconds_s));
		}
	...
	showCalendar : function() {
		...
		if (this.timeSeparators.length > 1) {
			...
			if (this.timeSeparators.length == 4) {
				if (this.selectedHour == 24 || this.selectedHour == 0)
					timeSelect += '<option value="00" selected="selected">00</option>';
				else
					timeSelect += '<option value="00">00</option>';
				for ( var a = 1; a < 24; a++) {
					if (this.selectedHour == a
							|| (a == 24 && this.selectedHour == 0)
							)
			...
			timeSelect += '</select>';
			var nowSecond_ = new Date().getSeconds();
			if (nowSecond_ <= 59) {
				timeSelect += ' <select id="calendar_second">';
				for ( var a = 0; a <= 59; a++) {
				if (a == nowSecond_)
					timeSelect += '<option value="' + a
							+ '" selected="selected">'
